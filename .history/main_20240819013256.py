...
def plot_png():
    ...
    ax.set_xlabel('날짜')  # x축 레이블 변경
    ax.set_ylabel('빈도수')  # y축 레이블 변경
    ax.set_title('화장실 사용 빈도수')  # 그래프 제목 변경
    
    # x축 날짜 형식 지정
    ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: pd.to_datetime(x).strftime('%m-%d')))
