...
def step2():
    ...
    # sensor_data5와 sensor_press_interval 데이터 가져오기
    sensor_data5 = get_sensor_data5()
    interval_alerts = []
    
    # sensor_press_interval 데이터를 가져와서 비정상적인 사용 시간 감지
    connection = get_db_connection(db_sensor_data3_config)
    cursor = connection.cursor(dictionary=True)
    
    query = "SELECT press_interval, timestamp FROM sensor_press_interval"
    cursor.execute(query)
    press_intervals = cursor.fetchall()
    
    cursor.close()
    connection.close()
    
    # 평균 계산
    intervals = [entry['press_interval'] for entry in press_intervals]
    avg_interval = sum(intervals) / len(intervals) if intervals else 0
    
    # 평균의 두 배를 초과하는 경우 경고 메시지 생성
    for entry in press_intervals:
        if entry['press_interval'] > avg_interval * 2:
            alert_message = f"비정상적인 사용 시간 감지: {entry['press_interval']}ms 초과"
            interval_alerts.append({
                'message': alert_message,
                'timestamp': entry['timestamp']
            })
    
    return render_template('step2.html', sensor_data5=sensor_data5, interval_alerts=interval_alerts)
