...
def analyze_duration_and_intervals():
    duration_data = get_sensor_duration()
    press_interval_data = get_sensor_press_interval()

    durations = [entry['duration'] for entry in duration_data]
    avg_duration = statistics.mean(durations)
    alerts = []

    for entry in duration_data:
        if entry['duration'] > avg_duration * 2:  # 비정상적으로 긴 사용 시간
            alerts.append({
                'type': 'duration',
                'timestamp': entry['timestamp'],
                'message': f"사용 시간이 평균보다 2배 이상 길었습니다: {entry['duration']}초"
            })
    
    for entry in press_interval_data:
        if entry['press_interval'] > 300_000:  # 예: 5분 이상 사용이 없을 경우 경고
            alerts.append({
                'type': 'interval',
                'timestamp': entry['timestamp'],
                'message': f"사용 간격이 너무 길었습니다: {entry['press_interval']}밀리초"
            })
...
def alerts():
    ...
    alerts = analyze_duration_and_intervals()
