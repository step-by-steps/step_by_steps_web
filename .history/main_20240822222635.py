...
def get_sensor_data(location):
    conn = pymysql.connect(**db_sensor_data5_config)

    try:
        with conn.cursor() as cursor:
            # location에 맞는 데이터를 가져오는 쿼리
            sql = "SELECT timestamp, event_type, location FROM sensor_events WHERE location=%s"            
...
def plot_png():
    ...
    if 'timestamp' not in df.columns:
        return "Error: 'timestamp' column not found in the data", 400
    ...
    if daily_data.empty:
        fig, ax = plt.subplots()
        ax.text(0.5, 0.5, '데이터가 없습니다', fontsize=12, ha='center')
        img = io.BytesIO()
        plt.savefig(img, format='png')
        img.seek(0)
        plt.close()
