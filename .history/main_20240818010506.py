...
def get_sensor_data1():
    connection = mysql.connector.connect(**db_sensor_data_config)
    cursor = connection.cursor(dictionary=True)
...
def step3():
    if 'username' not in session:
        return redirect(url_for('login'))
    analysis_text = "지난주에 비하여 어제보다 2배 정도 높아졌습니다."
...
def plot_png():
    ...
    # 날짜를 인덱스로 설정하고, 주별로 빈도를 계산
    df['timestamp'] = pd.to_datetime(df['timestamp'])
    df = df.set_index('timestamp')
    weekly_data = df.resample('D').size()
    
    # 그래프 생성
    fig, ax = plt.subplots()
    weekly_data.plot(kind='bar', ax=ax, color='blue')
    ax.set_xlabel('Date')
    ax.set_ylabel('Frequency')
    ax.set_title('Usage Frequency')
